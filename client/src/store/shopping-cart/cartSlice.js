  ...
  reducers: {
    ...
    addItem(state, action) {
      ...
      if (!existingItem) {
        ...
        let index = value.findIndex(s => s._id === existingItem._id);
        const newValue = {
          _id: existingItem._id,
        name: existingItem.name,
        image: existingItem.image,
        new_price: existingItem.new_price,
        quantity: 1,
      ...
      state.totalAmount = state.cartItems.reduce(
        (total, item) => total + Number(item.new_price) * Number(item.quantity),
        0
      );
    ...
    removeItem(state, action) {
      ...
      if (existingItem.quantity === 1) {
        ...
        existingItem.totalPrice =
          Number(existingItem.totalPrice) - Number(existingItem.new_price);
      }
